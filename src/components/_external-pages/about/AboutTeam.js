...
const MOCK_LEADERS = [...Array(3)].map((_, index) => ({
...
const MOCK_MEMBERS = [...Array(4)].map((_, index) => ({
  id: mockData.id(index + 3),
  name: mockData.name.fullName(index + 3),
  role: mockData.role(index + 3),
  avatar: mockData.image.avatar(index + 3)
}));
  ...
  return (
    <Container maxWidth="lg" sx={{ pb: 10, textAlign: 'center' }}>
      ...
      <MotionInView variants={varFadeInUp}>
        ...
        >
          TWebile will provide you support if you have any problems, our support team will reply within a day and we
          also have detailed documentation.
        </Typography>
      </MotionInView>

      <Box sx={{ position: 'relative' }}>
        <Slider ref={carouselRef} {...settingsL}>
          {MOCK_LEADERS.map((member) => (
